...
class GeminiService(private val apiKey: String = ApiConfig.GEMINI_API_KEY) {
    ...
    private suspend fun callGemini(jsonBody: String): String = withContext(Dispatchers.IO) {
        // Updated to V1 Stable endpoint with latest alias
        val url = "https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash-latest:generateContent?key=$apiKey"
        ...
        try {
            ...
            if (!response.isSuccessful) {
                "Error [V1-STABLE]: API call failed with code ${response.code}. Details: $responseBody"
            } else {
