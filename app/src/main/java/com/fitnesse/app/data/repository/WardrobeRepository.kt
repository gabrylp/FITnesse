...
class WardrobeRepository(
    ...
    suspend fun analyzeAndAddClothing(imageBytes: ByteArray, mimeType: String): Result<String> {
        return try {
            ...
            // WORKAROUND: Attempt storage upload, but continue if it fails (Free Tier)
            var photoUrl = ""
            try {
                photoUrl = firebaseRepo.uploadPhoto(imageBytes)
            } catch (e: Exception) {
                // Storage disabled/no billing, we continue with empty URL
                android.util.Log.e("WardrobeRepo", "Photo upload skipped (Storage disabled): ${e.message}")
            }
            ...
            Result.failure(Exception("Analysis failed: ${e.message ?: e.javaClass.simpleName}", e))
        }
    }
